...
library LibYell {
    function encodeYell(YellInfo memory yellInfo) internal pure returns (uint136) {
        return uint136(
            bytes17(abi.encodePacked(uint64(yellInfo.to), uint40(yellInfo.endValue), uint32(yellInfo.lifeSeconds)))
        );
    }

    function decodeYell(uint136 encodedYellInfo) internal pure returns (YellInfo memory) {
